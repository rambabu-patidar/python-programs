# Function is a block of code that runs when we call it otherwise not
...
# This way the function will receive a ** tuple **(that means  it is ordered,
...
fruits(
    fruit1="Orange", fruit2="Banana", fruit3="Pineapple", fruit4="Mango", fruit5="Kiwi"
)
# same order as parameter
fruits(
    fruit3="Orange", fruit5="Banana", fruit1="Pineapple", fruit4="Mango", fruit2="Kiwi"
)
# not in same order as parameter.
# both works fine.
...
defaultExample("India")
# use the default Indore
...
# Put a "pass" keyword if you have empty body in function
...
my_function(12)  # this is fine

# now if will try to give it a keyword argument then it will show you an error
# for eg.
my_function(
    x=100
...
# Similarly we can specify that a function can have only keyword arguments, add *, before the arguments:
def another_func(*, x, y):
    print(x, y)


another_func(x=100, y=20)  # fine

# now if you try to give it a positional argument then it will show you TypeError like below
another_func(100, 20)

# NOTE: when we don't specify anything then we can use any one of them : positional or keword args.
